...
public final class ChameleonExecutor {
	...
	/**
	 * Gets the instance of Chameleon, i.e. the OSGi Framework where the test are
	 * executed.
	 *
	 * @param root
	 *            the base directory of the Chameleon.
	 * @return the Chameleon Executor instance, newly created if none, or reuses if
	 *         any.
	 * @throws java.io.IOException
	 *             if the chameleon configuration cannot be read.
	 * @throws org.osgi.framework.BundleException
	 *             if the chameleon cannot be started.
	 */
	 ...
	 *
	 * @throws Exception
	 *             if the Chameleon instance cannot be stopped.
	 */
		...
		if (factor != null) {
			...
			if (factorAsInteger == 1) {
				LoggerFactory.getLogger(this.getClass())
						.info("Setting time.factor to " + factorAsInteger + " (given " + "as '" + factor + "')");
			} else {
	...
	/**
	 * @return the bundle context of the underlying Chameleon, {@literal null} if
	 *         not started.
	 */
	...
	/**
	 * Stops the underlying chameleon instance. If we have an application artifact,
	 * restore it in the 'application' directory. It may have been deleted from the
	 * 'application' directory before running the integration tests.
	 *
	 * @throws Exception
	...
	/**
	 * Deploys the `probe` bundle, i.e. the bundle containing the test classes and
	 * the Wisdom Test Utilities (such as the InVivo Runner). If such a bundle is
	 * already deployed, nothing is done, else, the probe bundle is built, installed
	 * and started.
	 * <p>
	 * Initially, this method was returning {@code null}. In the 0.7 version, it
	 * changes to {@code Bundle}. The returned object is the installed bundle.
	 *
	 * @return the probe bundle.
	 * @throws BundleException
	 *             if the probe bundle cannot be started.
	 */
	...
	/**
	 * Builds and deploy the application bundle. This method is called the
	 * application bundle is not in the runtime or application directories.
	 */
	public static void deployApplication() throws BundleException {
		...
		try {
			Bundle app = ChameleonInstanceHolder.get().context()
					.installBundle(application.toURI().toURL().toExternalForm());
			app.start();
		} catch (Exception e) {
	 ...
	 * @return the runner
	 */
	public InVivoRunner getInVivoRunnerInstance(Class<?> clazz)
